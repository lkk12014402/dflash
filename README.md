...
Train your own DFlash draft to accelerate any supported target. The recipe is
EAGLE-style but adapted to DFlash's *feature-conditioned block diffusion*: the
draft never re-processes tokens, it cross-attends to features extracted from the
frozen target and denoises a fully-masked block in one bidirectional forward.
The training forward is **bit-equivalent to the inference forward** (verified by
the self-test below), so what you train is exactly what you run.

**1. Verify the training/inference equivalence (recommended first):**
```bash
CUDA_VISIBLE_DEVICES=0 python -m dflash.train selftest \
    --model Qwen/Qwen3-4B --draft-model z-lab/Qwen3-4B-DFlash-b16 --fp32
# -> 100% argmax agreement between the whole-sequence trainer and per-block decode
```

**2. Generate target-greedy training data.** Training is inference-equivalent only
on sequences from the target's own decoding trajectory, so we let the target
greedily continue prompts and supervise on those tokens (hard CE on greedy data
== distilling the temperature-0 verifier). The DFlash paper builds its training
set the same way: ~800K prompts mixed from **NVIDIA Nemotron-Post-Training-Dataset-V2**
and **CodeAlpaca**, with the **responses regenerated by the target model** for
alignment (the released LLaMA-3.1 draft uses UltraChat). Point `--prompts-jsonl`
at any prompt mixture you like:
```bash
...
```
You can also pass `--prompts-jsonl` with custom `{"prompt": ...}` / `{"messages": [...]}` rows.

**3. Train.** Start from a released draft (`--init-from`) to fine-tune, or from a
config (`--draft-config`) to train a fresh draft:
```bash
# single GPU
CUDA_VISIBLE_DEVICES=0 python -m dflash.train train \
    --model Qwen/Qwen3-4B --data cache/train_gsm8k.jsonl \
    --draft-config z-lab/Qwen3-4B-DFlash-b16 \
    --output-dir dflash_ckpt --batch-size 2 --grad-accum 8 --epochs 2 --lr 1e-4

# multi-GPU (DDP)
torchrun --nproc_per_node=8 -m dflash.train train \
    --model Qwen/Qwen3-4B --data cache/train_gsm8k.jsonl \
    --draft-config z-lab/Qwen3-4B-DFlash-b16 --output-dir dflash_ckpt
```
Only the draft parameters are trained; the target's (tied) `embed_tokens` and
`lm_head` stay frozen and provide the embeddings/vocab projection. The CE loss is
weighted toward early block positions (`--loss-decay`, DFlash eq. 4) since an
early error invalidates all later tokens. Optional `--distill-weight` adds a
correctly-shifted KL against the target's own logits on top of the hard
cross-entropy. The saved checkpoint loads directly with
`DFlashDraftModel.from_pretrained(...)` for benchmarking.
